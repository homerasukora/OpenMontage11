...
/**
 * A void shot is not flat black — a defocused nebula plate keeps it alive.
 * This episode uses it twice, both times deliberately: when the film shows
 * you something we actually sent, the sky goes out and a diagram draws
 * itself on the dark. It is the one repeated gesture in the cut.
 */
export const Void: React.FC<{dur: number}> = ({dur}) => {
  ...
  return (
    <AbsoluteFill style={{opacity: settle * 0.62, overflow: 'hidden'}}>
      <Img
        src={staticFile('void_bg.jpg')}
        style={{
          width: '100%', height: '100%', objectFit: 'cover',
          transform: `scale(1.12) translateY(${drift}px)`,
          filter: 'brightness(0.5) contrast(1.0) blur(3px)',
        }}
      />
    </AbsoluteFill>
  );
