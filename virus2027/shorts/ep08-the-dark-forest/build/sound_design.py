#!/usr/bin/env python3
"""
Sound design for TRANSMISSION 08 — THE DARK FOREST.
...
def main():
    ...
    # One hit per graphic, chosen by what the graphic is. From ep08 the
    # drawn elements are all type "viz" and carry their real identity in
    # "kind", so that is what the cue table reads; the older stamp/card
    # names are kept so an earlier episode's beat sheet still scores.
    for b in beats:
        if b["at"] not in start:
            continue
        t = start[b["at"]] + b["lead"]
        kind = b.get("kind") if b["type"] == "viz" else b["type"]

        # The pulsar map draws fourteen rays, one every 2.1 frames from
        # frame 10. Each gets its own tick. Fourteen small dry clicks in
        # just under a second is the sound of something being catalogued,
        # and it is the only busy moment in an otherwise very quiet mix.
        if kind == "pulsarmap":
            for k in range(14):
                place(fx, tick(rng, 0.06), t + (2 + k * 2.1) / 30.0, 0.30)
            place(fx, impact(rng, 0.5, 48), t + (2 + 14 * 2.1) / 30.0, 0.24)
            continue
        if kind == "helix":
            place(fx, whoosh(rng, 0.62), t - 0.10, 0.20)
            continue
        if kind == "flatline":
            place(fx, whoosh(rng, 0.7), t - 0.12, 0.12)
            continue
        if kind == "figure":
            place(fx, tick(rng, 0.09), t, 0.34)
            continue
        if kind == "rings":
            place(fx, impact(rng, 0.8, 42), t, 0.22)
            continue
        ...
        elif kind in ("dossier", "genome"):
            place(fx, whoosh(rng, 0.38), t - 0.12, 0.26)
            place(fx, tick(rng), t, 0.4)
        elif kind == "reticle":
            # brackets closing, then the lock at frame 24
            place(fx, whoosh(rng, 0.80), t - 0.08, 0.26)
            place(fx, tick(rng, 0.10), t + 24 / 30.0, 0.52)
            place(fx, impact(rng, 0.40, 54), t + 24 / 30.0, 0.22)
        elif kind == "redact":
            place(fx, whoosh(rng, 0.55), t, 0.34)
        elif kind in ("tag", "place"):
            place(fx, tick(rng, 0.07), t, 0.3)

    # Every picture change gets a transient so the edit is felt as well as
    # seen — but a camera move onto the same painting is not a cut, so a pano
    # shot only gets the air, not the thud.
    for i, sh in enumerate(shots):
        if i == 0 or sh["at"] not in start:
            continue
        t = start[sh["at"]] + sh["lead"]
        if sh["type"] == "pano":
            place(fx, whoosh(rng, 0.46), t - 0.20, 0.20)
        else:
            place(fx, whoosh(rng, 0.34), t - 0.14, 0.24)
            place(fx, impact(rng, 0.34, 66), t, 0.20)
    ...
    last_line = ids[-1]
    brand_t = (start[brand_at["at"]] + brand_at["lead"]) if brand_at else start[last_line]
    place(fx, riser(rng, 1.7), start[last_line] - 1.7, 0.5)
    place(fx, impact(rng, 1.1, 46), brand_t, 0.6)
    place(fx, tick(rng), brand_t + 0.85, 0.5)
